...
LP8::LP8(gpio_num_t vbb_en_pin /*= GPIO_NUM_5*/, gpio_num_t rdy_pin /*= GPIO_NUM_18*/, uart_port_t uart_port /*= UART_NUM_2*/) {
    _first_sense = false;
    _uart_port = uart_port; // puerto predefinido
...
void LP8::SendRequest(const uint8_t *packet) {
    ...
    if (_response[1] == 0x41) {
        size_receive = 4;  // Solo 4 bytes para comando 0x41
    } else if (_response[1] == 0x44) {
        size_receive = 49; // 49 bytes para comando 0x44
    } else {
        printf("Advertencia: comando desconocido 0x%02X\n", _response[1]);
        size_receive = 4; // Valor seguro por defecto
    }
  ...
  start_time = esp_timer_get_time();
    ...
    while (i < size_receive && ((esp_timer_get_time() - start_time) < (_time_out_ms * 1000))) {
